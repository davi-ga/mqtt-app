...
def illumination_send() -> None:
    ...
    mqtt_class = mqtt.Client('illumination')
    mqtt_class.connect(host = settings.MQTT_BROKER_HOST,
                       port = 1883)
    mqtt_class.publish(topic = 'illumination',
...
def humidity_send() -> None:
    ...
    mqtt_class = mqtt.Client('humidity')
    mqtt_class.connect(host = settings.MQTT_BROKER_HOST,
                       port = 1883)
    mqtt_class.publish(topic = 'humidity',
                       payload = humidity)
...
def water_level_send() -> None:
    ...
    mqtt_class = mqtt.Client('water_level')
    mqtt_class.connect(host = settings.MQTT_BROKER_HOST,
                       port = 1883)
    mqtt_class.publish(topic = 'water_level',
                       payload = water_level)
...
def sound_send() -> None:
    ...
    mqtt_class = mqtt.Client('sound')
    mqtt_class.connect(host = settings.MQTT_BROKER_HOST,
                       port = 1883)
    mqtt_class.publish(topic = 'sound',
                       payload = sound)
